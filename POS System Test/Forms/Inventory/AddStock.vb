...
Public Class AddStock
    ...
    Private Sub btAddStock_Click(sender As Object, e As EventArgs) Handles btAddStock.Click
        If String.IsNullOrEmpty(txbxID.Text.Trim) OrElse String.IsNullOrEmpty(txbxPrice.Text.Trim) OrElse String.IsNullOrEmpty(txbxQty.Text.Trim) OrElse String.IsNullOrEmpty(txbxProduct.Text.Trim) Then
            MessageBox.Show("Please fill in given fields")
            Exit Sub
        Else
            If dgvAddDescr.Rows.Count > 0 Then
                For Each row In dgvAddDescr.Rows
                    If IsDBNull(row.Cells(1).Value) OrElse String.IsNullOrWhiteSpace(row.Cells(1).Value.ToString()) Then
